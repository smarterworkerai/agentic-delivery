...
If a gate is rejected, failed, pending, or stale, or review, target identity, secret preservation, configuration or deployment evidence is unresolved, **stop and report the blocker**; do not promote until it is remedied and revalidated. Expected implementation commits within the approved feature scope do not alone invalidate the initial grant; they must pass the ordinary new-tree quality, review and preview gates before promotion. A change to an already reviewed or validated tree invalidates its old proof: pause the promotion, rerun the gates on the new tree, and continue under the original grant only if the feature scope and route are unchanged. The grant has no time-based expiry and remains valid across successful stages of the exact route. Completion ends it; a changed target/route, unapproved scope change, or interruption that makes its exact scope unverifiable requires a new, explicit proposal and authorization before continuation. On resume, reconstruct the exact approved scope and current evidence; if the grant cannot be verified, ask again. Never silently waive a gate, rewrite history, rollback, change secrets, run a destructive operation, or substitute a different target under this grant. Production merge and deployment are high-risk actions covered only by the explicit first confirmation naming them, never by inference.
