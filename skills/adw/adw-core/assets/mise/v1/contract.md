...
Project and context task implementations should invoke `adw_contract.py run --task <task> [--environment <value>] [--child <canonical-task>]... -- <command>`. The adapter validates support and environment scope before execution, preserves a nonzero child exit as `failed`, persists only the selected target environment name and declared canonical child task names, and does not persist child command arguments or environment-variable values. Only aggregate tasks may declare unique, non-self-referential children. Unsupported generic stubs use `adw_contract.py unsupported`, return zero, and record `unsupported`.

`ADW_RUN_ID` is inherited when supplied and generated otherwise. Aggregate and child records share the run ID and identify relationships. Evidence records contract/schema version, task, status, exit code, source revision, effective task source/ref/checksum, arguments, timestamps, children, and findings.
